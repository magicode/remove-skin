...
public:
	...
	static Handle<Value> Add(const Arguments& args) {
		...
		if(Buffer::Length(buffer_obj) != 512){
			return ThrowException( Exception::TypeError( String::New("Buffer::Length() != 512")));
		}

		uint pos = (obj->position + 1) % obj->maxLength;


		memcpy( obj->tableCmp[pos].data ,Buffer::Data(buffer_obj),512);
...
public:
	...
	static void DetectWork(uv_work_t* req) {
		...
		FIBITMAP * thumbnail2 = FreeImage_ConvertTo4Bits(thumbnail1);
